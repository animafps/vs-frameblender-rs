...
mod plugin {
    ...
    impl Filter for FrameBlender {
        ...
        fn process_frame<'core>(
            ...
            for _ in 0..=self.weights.len() {
                let frame = self.input_node.get_frame_filter(max(0, frame_num), &frame_ctx).unwrap();
    ...
    impl FrameBlender {
        ...
        fn frame_blend<T>(&self, srcs: &Vec<Frame>, dst: &mut Frame, plane: i32) where T: TryFrom<i32> + Copy + Into<f64> , <T as TryFrom<i32>>::Error: Debug {
                ...
                for _h in 0..height {
                    for w in 0..width as usize {
                        ...
                        unsafe { dst_ptr.wrapping_add(w).write(actual_acc.try_into().unwrap()); }
                    }
                    for i in 0..num_srcs {
